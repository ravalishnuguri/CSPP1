...
score for a single word. The function get_word_score should accept as input a string
...
def get_word_score(word, number):
    ...
    scrabblelettersvalue = {'a': 1, 'b': 3, 'c': 3, 'd': 2, 'e': 1, 'f': 4,\
    ...
    while index < len(str1):
        points = points + scrabblelettersvalue[str1[index]]
        index = index + 1
    result = points * len(str1)
    if len(str1) == number:
        return result + 50
    return result

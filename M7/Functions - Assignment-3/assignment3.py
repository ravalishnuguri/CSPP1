...
# Does your code still work? It should, but you may notice that your code runs more
#slowly, especially in cases with very large balances
...
#in Problem 2 without running into the problem of slow
# code?
...
def calculate(month, bal, minpay, mir):
    '''doc'''
    while month < 12:
        unpaidbal = bal - minpay
        bal = unpaidbal + (mir * unpaidbal)
        month += 1
    return bal



def payingdebtoffinayear(bal, air):
    '''d'''
    initbalance = bal
    mir = air/12.0
    low = bal/12.0
    high = (bal * ((1.0 + mir)** 12))/12.0
    epsilon = 0.01
    minpay = (high + low)/2.0
    month = 0
    while abs(bal) >= epsilon:
        bal = initbalance
        month = 0
        bal = calculate(month, bal, minpay, mir)
        if bal > 0:
            low = minpay
        else:
            high = minpay
